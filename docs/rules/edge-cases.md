...
- **Autopilot:** quando o jogador da vez está desconectado, o server joga por ele com a IA `oficial` (uma ação a cada ~900ms, ~60ms no setup) até reconectar — a partida nunca congela por rage-quit.
- Quando a partida termina, assentos offline saem do roster; `start` exige ≥2 conectados.
- Sala mid-game sem ninguém conectado continua viva (autopilot segue); o sweep remove salas inertes por >2h.
