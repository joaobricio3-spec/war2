...
region = poly ∩ landmask(board art). The board then draws each territory as
a Sprite of its region PNG (white+alpha, tintable to owner color) so the
colored area follows the painted coastline instead of the chunky polygon.
...
def main():
    ...
    lm = to_img(land).filter(ImageFilter.MedianFilter(5))
    lm = lm.filter(ImageFilter.MaxFilter(3)).filter(ImageFilter.MinFilter(3))
